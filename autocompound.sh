...
for (( ;; )); do
        ...
        echo -e "${password}\ny\n" | stchaind tx distribution withdraw-rewards ${VALIDATOR_ADDRESS} --commission --gas="auto" --gas-adjustment="1.15" --gas-prices="0.5ustos" --chain-id tropos-5 --from ${KEY_NAME} --node ${NODE} --yes | grep "raw_log\|txhash"
...
echo -e "BALANCE: ${GREEN_COLOR}${BALANCE}${WITHOUT_COLOR} wei\n"
        ...
        echo -e "${password}\n${password}\n" | stchaind tx distribution withdraw-all-rewards --gas="auto" --gas-adjustment="1.15" --gas-prices="0.5ustos" --chain-id tropos-5 --from ${KEY_NAME} --node ${NODE} --yes | grep "raw_log\|txhash"
for (( timer=10; timer>0; timer-- ))
        do
                printf "* sleep for ${RED_COLOR}%02d${WITHOUT_COLOR} sec\r" $timer
                sleep 1
...
if awk "BEGIN {return_code=($BALANCE > $ONE_STOS) ? 0 : 1; exit} END {exit return_code}";then
            echo -e "${password}\n${password}\n" | stchaind tx staking delegate ${VALIDATOR_ADDRESS} ${TX_AMOUNT}wei --gas="auto" --gas-prices="0.5ustos" --gas-adjustment="1.15" --chain-id=tropos-5 --from ${KEY_NAME} --node ${NODE}  --yes | grep "raw_log\|txhash"
        else
